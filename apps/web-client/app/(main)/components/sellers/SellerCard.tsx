...
export const SellerCard = ({ seller }: { seller: Sellers }) => {
  return (
    <div className="shrink-0 relative w-[210px] lg:w-[220px] h-80 lg:h-60 bg-[#F5F5F6] text-[#1F242F] rounded-[12px] pb-6 px-4 flex flex-col items-center justify-end gap-2  hover:shadow-sm transition-all duration-300">
      <div className="relative lg:absolute -top-4 lg:-top-14 rounded-full w-[81px] h-[81px]  lg:w-[100px]  lg:h-[100px] flex items-center justify-center ">
        {/* <Image src={seller.thumbnailImage} alt={seller.name} fill /> */}
        <Image src={"/svg/home/Avatar.svg"} alt={seller.name} fill />
      </div>

      <div className="w-full flex flex-col items-start gap-1">
        <h3 className="font-medium text-base">{seller.name}</h3>
        <p className="text-xs font-medium text-[#61646C]">
          به مدیریت {seller.owner.fullName}
        </p>
      </div>
        ...
        <div className="flex items-center gap-1">
          {Array.from({ length: 5 }, (_, i) => (
            <Star
              key={i}
              size={12}
              className={i < 5 ? "text-[#FDB022]" : "text-[#CECFD2]"}
              fill={i < 5 ? "#FDB022" : "#CECFD2"}
            />
          ))}
        </div>

        <div className="text-xs font-medium">
          {seller.shopReviews.length} نظر
        </div>
      </div>

      <div className="w-full flex items-center justify-between gap-1 text-xs font-medium">
        <span className="text-xs text-[#85888E]">تعداد محصول</span>
        <div className="">{seller.productsCount} عدد</div>
      </div>

      <div className="w-full flex items-center justify-between gap-1 text-xs font-medium">
        <span className="text-xs text-[#85888E]">محل شرکت</span>
        <div className="">{seller.owner.location}</div>
      </div>
