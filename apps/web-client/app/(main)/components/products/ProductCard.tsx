...
export const ProductCard = ({ product }: { product: Product }) => {
  return (
    <div
      dir="rtl"
      className=" min-w-[213px] w-[213px] max-w-[213px] bg-[#FAFAFA] text-[#333741] border rounded-[12px]  p-2 flex flex-col gap-2 hover:shadow-lg transition-all"
    >
      ...
      <div className="w-full flex items-center justify-between gap-1 text-xs font-thin border-t pt-3">
        ...
        <div className="">فروشگاه {product?.sellers[0]?.seller?.name}</div>
      </div>

      <div className="flex justify-between items-center gap-2 mt-">
        <Button
          asChild
          className="grow px-6! h-8  rounded-[3px] bg-brand-primary hover:bg-[#e7bd35] text-[#1C1D1F] text-[11px] font-medium"
        >
