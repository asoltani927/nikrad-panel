...
// export default function OtpForm({ phone }: Props) {
...
//     const otpSchema = z.string().length(6, "کد تأیید باید ۶ رقمی باشد");
...
export default function OtpForm({ phone }: Props) {
  ...
  const { verifyOtp, loading, error: authError, user } = useAuth();
  ...
  const handleNext = async () => {
    ...
    const ok = await verifyOtp(phone, otpValue);
    if (!ok) setError(authError || "خطا در تأیید کد");
  };
    ...
    <div className="w-full h-screen flex items-center gap-6 bg-white py-4 px-8 lg:px-0 lg:pe-4">
      ...
      <section className="relative h-full flex items-center justify-center w-full lg:w-[43%]">
        ...
        <div className="relative w-full flex flex-col items-center justify-center z-20">
          ...
          {(error || authError) && (
            <div className="w-[275px]">
              <p className="text-red-500 text-[12px] mb-2">{error || authError}</p>
            </div>
          )}
          ...
          <Button
            disabled={loading}
            onClick={handleNext}
            className="w-full lg:w-[320px] h-10 lg:h-9 text-[16px] cursor-pointer rounded-sm mt-1 font-light bg-[#EAAA08] hover:bg-[#d8a708] disabled:opacity-50"
          >
            {loading ? "در حال ارسال..." : "ورود به حساب"}
          </Button>
        </div>
